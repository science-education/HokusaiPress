...
## 2026-07-05(2): 柱の混入と縦錨の基準混在を修正(本文枠の左右偏りの真因)
- ユーザー実測(8頁: 左10右6)から特定: 柱(小口上隅)が本文と横方向に重なり
  x-span解析で除去されず、さらに本文が節見出し空白でy-span分裂して支配スパン不成立
  → y-pass不発で hull が柱の分だけ小口側に伸び、**左右頁で±4〜5mmの鏡映ずれ**。
- 修正1: body_text_box の y-pass を「支配スパン要求なし・外端の極小スパン
  (<4%高)+大ギャップ(≥4%)のみ除去」に変更(OUTER_FURNITURE_*)。章冒頭の
  題字ブロックは誤除去しない。内側の見出しも保持(テスト2件追加)。
- 修正2: 縦錨の基準混在 — 下マージンをhull基準で測りノンブル基準で適用していたため
  柱が紙端1mmまで押し上げられていた → ノンブル基準の実測値(nb_bottom/top_space)で
  測定・適用を統一。柱の上余白 8.6mm に復元。
- 実書籍検証: 本文帯L-R 全122頁 median +0.2mm、両パリティとも +0.2mm(鏡映解消)。
  318 passed。可視化: tmp/margin_debug/standard_frame_sheet2.png。
...
- 編集系エンドポイント（decide/region/content/nombre 等）の所有権チェックは未実装（次 WP 対象）。
...
## 2026-07-05: マージン正規化を「版面(printing-plate)モデル」に刷新(ユーザー設計)
- ユーザー提案のアルゴリズムを採用: 満ページ(hull が基準サイズの90%以上)のみの頑健統計で
  標準本文枠と**標準ノンブル点**(パリティ別の e_x = ノンブル中心−版面中心)を決め、
  ノンブル頁は `crop中心 = ノンブル中心 − e_x` で配置。章末の部分ページは hull でなく
  **版面が正位置に復元**され、途中終わりの文章は組版どおりの位置に残る。
- 偽ノンブルガード: パリティ典型位置(ページ幅比)から0.15超ずれたノンブルは錨に使わず
  hull中央化へフォールバック。無ノンブル頁は比例配置(従来どおり)。垂直錨も従来どおり。
- 実書籍検証: 全122ノンブル頁でノンブルが標準点に **ばらつき median 0.4px / max 1.1px**
  で着地(パリティ鏡映 +680/−681px)。満ページ hull L-R median 0。
- 前日(07-03)の「hull左右中央化」はこのモデルに置換された(部分ページを誤って中央へ
  引きずる欠陥があった)。tests/test_margin_normalize.py に版面モデルの3テスト、計316 passed。

## 2026-07-03: マージン正規化を「本文枠の左右中央」に変更(→ 07-05に版面モデルへ置換)
