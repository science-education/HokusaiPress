...
## 8.5 リッチ領域分類・ベクタ還元・ページ役割（構想 2026-06-16）
最終的に各領域を **文字 / 線画・表(線分中心) / べた塗り網掛け・図形 / 写真 / その他** に分類し、
クラス別に最適処理する。特に **べた塗り網掛けは PDF ベクタ図形(矩形+濃度)へ還元** すると容量大幅減＋
鮮明（課題E網掛けも"除去"でなく"ベクタ化"で解決）。地色＋文字は MRC にベクタ背景層を足す。
- 判別: 線画=高エッジ密度・低充填率／べた塗り=高充填率・低内部エッジ・単一トーン（画像統計）。
- **ページ役割推定**（表紙/図版/章扉/本文/コラム/索引/奥付）を要素の配置・割合＋構造内位置から。
  既存 page_kind＋learn.py（判断ログ→教師あり）を土台に、規則ベース→学習で精緻化。
- **目次OCRが役割推定の最強信号**: 目次の「章タイトル＋頁番号」対から **章扉ほぼ確定・見出し~8割取得・
  コラム列挙**、**柱(ヘッダ)と突き合わせ構造検証**。本文テキストは保存せず役割/構造のみ保存（著作権セーフ）。
- **2段階推定**: 書籍レベル（要素ミックス・役割レパートリ）↔ ページレベル（役割, 書籍priorで制約）。
  partial pooling の自然な拡張。OCR/layout-provider のクラスは再利用、無い区別だけ推定を足す。
- 段階: (a)figure→線画/べた塗り分割 (b)べた塗り→ベクタ還元 (c)ページ役割推定 (d)役割/要素を書籍プロファイルへ。
